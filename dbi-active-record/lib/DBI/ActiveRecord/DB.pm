...
sub update {
    ...
    for (@to_update) {
        my $attr = $obj->meta->get_attribute($_);
        push @bind, ( $attr->serializer ? $attr->serializer->($obf->$_) : $obj->$_ );
    }
    ...
    my $update_result = $self->_update($obj->meta->table_name, $key_field, $key_value, \@to_update, \@bind);

    unless (defined $update_result) {
        confess "update failure!";
    }
